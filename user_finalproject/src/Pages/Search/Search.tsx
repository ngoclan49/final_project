...
import { getRoomByPosIdApi, getRoomPagination } from "../../redux/roomReducer/roomReducer";
...
const Search = () => {
  ...
  const { roomByPos, roomPagination, rooms } = useSelector(
  ...
  const handleNextPage = () => {
    const hasNextPage = pageNumber === pageCount ? false : setPageNumber(prevPage => prevPage + 1);
    return hasNextPage
  }
  ...
  return (
    <main className="px-3">
      <section className="pt-5">
        ...
        <h1 className="font-bold mt-2 mb-4 fs-4">Stay in {position || 'anywhere'}</h1>
        <div
          style={{ color: "#909090" }}
        ...
        <div className="search--container d-flex justify-content-between">
            ...
            {roomByPos.length == 0 ? (
              <>
                <div className="text-center">
                  <p className="mb-0" style={{ color: '#909090' }}>There is no accommodation in this location. Please refer to other accommodation</p>
                </div>
          ...
          <div style={{height: '100% !important', width: '100%'}} className="pt-3 map">
            <Map />
          </div>
        </div>
      </section>
